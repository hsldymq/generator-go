...
func Once[T any](it Iterator[T]) Iterator[T] {
	flag := int32(0)
...
func Once2[T1, T2 any](it Iterator2[T1, T2]) Iterator2[T1, T2] {
	flag := int32(0)
...
func ContinuableOnce[T any](it Iterator[T]) Iterator[T] {
	return continuable(Once(it))
}

// ContinuableOnce2 is the Iterator2 version of ContinuableOnce.
func ContinuableOnce2[T1, T2 any](it Iterator2[T1, T2]) Iterator2[T1, T2] {
	return continuable2(Once2(it))
}

func continuable[T any](it Iterator[T]) Iterator[T] {
	stopped := false
	...
	pull := func() (func() (T, bool), func()) {
		if nextFunc == nil {
			stopped = false
			nextFunc, stopFunc = iter.Pull(it.Seq())
...
func continuable2[T1, T2 any](it Iterator2[T1, T2]) Iterator2[T1, T2] {
	stopped := false
	...
	pull := func() (func() (T1, T2, bool), func()) {
		if nextFunc == nil {
			stopped = false
			nextFunc, stopFunc = iter.Pull2(it.Seq())
			stopFunc = sync.OnceFunc(stopFunc)
		}
		return nextFunc, stopFunc
	}
